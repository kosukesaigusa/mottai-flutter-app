...
Future<void> main() async {
  ...
  group('現在バージョンが最低限バージョンより小さいかを判定するメソッドのテスト', () {
    test(
      ...
      () => expect(
        isCurrentVersionLessThanMinRequiredVersion('0.0.1', '0.0.2'),
        true,
      ),
    );
    test(
      ...
      () => expect(
        isCurrentVersionLessThanMinRequiredVersion('0.1.0', '0.1.0'),
        false,
      ),
    );
    test(
      '現在バージョン>最低限バージョンはfalse',
      () => expect(
        isCurrentVersionLessThanMinRequiredVersion('1.0.0', '0.0.1'),
        false,
      ),
    );
  });
